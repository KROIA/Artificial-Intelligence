...
void 	Neuron::setup(int in,float fac){
	...
	inputs[in].weight 	= randEngine() %100;
	inputs[in].weight 	= (inputs[in].weight+1) / 100;
	inputs[in].last_weight 	= inputs[in].weight ;
	for( a = 0; a<in; a++)
	{
		inputs[a].data 	= 0.0;	
		inputs[a].weight= randEngine() %100;		// Init Wert für das Gewicht aller Eingänge
		inputs[a].weight= (inputs[a].weight+1)/100;
...
Net::Net(int inputs,int hiddenX,int hiddenY,int outputs,float fac){
	...
	for(b = 0; b<animals; b++)
	{
		...
		{
			Genetic[b][a] = randEngine() %100;
			Genetic[b][a] = (Genetic[b][a]+1)/100;
		}
	}
	#endif
	//---------------------------------------------
...
//#define D	
		...
		{
			b = 0;
			randomSelection1 = randEngine() % (gesamtScore);
			randomSelection1 = (randomSelection1/10);
			randomSelection2 = randEngine() %(gesamtScore);
			randomSelection2 = (randomSelection2/10);
			#ifdef D
			printf("Random1: %.1f\nRandom2: %.1f\n",randomSelection1,randomSelection2);
			#endif
			while(randomSelection1 > gs)
			{
				randomSelection1 = randEngine() % (gesamtScore);
				randomSelection1 = (randomSelection1/10);
			}
			while(randomSelection2 > gs)
			{
				randomSelection2 = randEngine() %(gesamtScore);
				randomSelection2 = (randomSelection2/10);
			}
			...
			while(b == 0)
			{
				...
				if(SelectedAnimal[0] >= 0 && SelectedAnimal[1] >= 0)
				{
					...
					for(d = 0; d<2; d++)	// 2 animals
					{
						if(d == 0)
						{
							randCrossoverPoint= randEngine() %geneticSize;
							buffCrossoverPoint= 0;
						}
						else
						{
							randCrossoverPoint= geneticSize;
							buffCrossoverPoint= randEngine() %geneticSize;
						}
						#ifdef D
				...
				{
					printf("rand1: %.3f \nrand2: %.3f\n",randomSelection1,randomSelection2);
					randomSelection1 = randEngine() % (gesamtScore-10);
					randomSelection2 = randEngine() % (gesamtScore-10);
					randomSelection1 = (randomSelection1/10)+1;
					randomSelection2 = (randomSelection2/10)+1;
				}
			}
		}
		for(a = 0; a < animals; a++)
		...
		if(mutationRate != 0)
		{
			...
			for(a = 0; a < animals; a++)
			{
				for(b = 0; b < geneticSize; b++)
				{
					mutationRange = (randEngine() %mutationRate);
					
					if(mutationRange == 0)
					{
						tmpMutation = randEngine()%100;	//Mutiert zu random wert
						Genetic[a][b] += (tmpMutation-50) /10000;
					}
				}
			}
		}
