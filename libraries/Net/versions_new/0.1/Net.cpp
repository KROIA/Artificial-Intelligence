...
//--------------------------NEURON--------------------------
				Neuron::Neuron(int anzInputs){
					#ifdef DEBUG2
					printf("Neuron::Neuron\n");
					#endif
	inputs					= anzInputs;
	randEngine				= default_random_engine(rand()%500);
	weight 					= vector<float>	(inputs,(randEngine()%100)/100);
}
				Neuron::~Neuron(){
	weight.clear();
}
...
//----------------------------------------------------------
//--------------------------NET-----------------------------
				Net::Net(int in,int hidX,int hidY,int out,int ani,float fac,bool _bias){
					#ifdef DEBUG2
					printf("Net::Net\n");
					#endif
	inputs 					= in;
	...
	if(bias)
	{		//----------------------- +1 wegen dem BIAS
		genomSize 			= (inputs+1) * hiddenY + (hiddenY+1) * hiddenY * (hiddenX-1) + (hiddenY+1) * outputs;
	}
	else
	{
		genomSize 			= inputs * hiddenY + hiddenY * hiddenY * (hiddenX-1) + hiddenY * outputs;
	}
	//printf("net: %i %i %i %i\n",inputs,hiddenY,hiddenX,outputs);
	//printf("genomSize %i \n",genomSize);
	//getchar();
	//-------------------------------------------Erstelle das Array aus Neuronen	
	if(bias)
	{
		hiddenLayer				= vector<vector<Neuron>	>(hiddenX,vector<Neuron>(hiddenY+1,Neuron(hiddenY+1)));
		hiddenLayer[0]			= vector<Neuron>(hiddenY,Neuron(inputs+1));
		outputLayer 			= vector<Neuron>(outputs,Neuron(hiddenY+1));
	}
	else
	{
		hiddenLayer				= vector<vector<Neuron>	>(hiddenX,vector<Neuron>(hiddenY,Neuron(hiddenY)));
		hiddenLayer[0]			= vector<Neuron>(hiddenY,Neuron(inputs));
		outputLayer 			= vector<Neuron>(outputs,Neuron(hiddenY));
	}
	//--------------------------------------------------------------------------
	/*printf("%.8f\n",((randEngine()%100)));
	printf("%i\n",((randEngine()%100)));
	printf("%.8f\n",((randEngine()%100)+1)/100);
	printf("%.8f\n",((randEngine()%100)+1)/100);
	printf("genomSize: %i\n",genomSize);
	getchar();*/
...
void 			Net::saveData(char *_file){
	...
	rewind (file);
	fprintf(file,"%i %i %i %i %i %i\n",inputs,hiddenY,hiddenX,outputs,animals,bias);
	for(int a=0; a<animals; a++)
	{
		for(int b=0; b<genomSize; b++)
		{
...
void			Net::loadData(char *_file){
	...
	if(tmpInputs!=inputs || tmpHiddenY!=hiddenY || tmpHiddenX!=hiddenX || tmpOutputs!=outputs || tmpAnimals!=animals || tmpBias!=bias)
	{
		#ifdef DEBUG
		handleError(13);
		#endif
...
void			Net::learn(vector<float> fitness){
	...
	for(int x=0; x<animals/2; x++)
	{
		...
		*/
		...
		for(int a=genomSize-randomCrossoverPoint-1; a<genomSize; a++)
		{
