...
void QtGeneticNet::draw(QPainter *painter,QPoint drawPos,float colorMultiplayer,unsigned int animal)
{
    if(!lockForWrongData)
    {
        ...
        //qDebug("outputs: %i",_outputs);

       // std::vector<Line>                   inputConnection     ((_inputs+_enableBias)*_hiddenY,Line());
      //  qDebug("QtGeneticNet::draw inputConnection new line");
        std::vector<std::vector<Line>   >   inputConnection     (_inputs+_enableBias,std::vector<Line>(_hiddenY,Line()));
       // std::vector<std::vector<Line>   >   hiddenConnection    (_hiddenX-1,std::vector<Line>((_hiddenY+_enableBias)*_hiddenY,Line()));
     //  qDebug("QtGeneticNet::draw hiddenConnection new line");
       ...
       // std::vector<Line>                   outputConnection     ((_hiddenY+_enableBias)*_outputs,Line());
      // qDebug("QtGeneticNet::draw outputConnection new line");
        std::vector<std::vector<Line>   >   outputConnection     (_hiddenY+_enableBias,std::vector<Line>(_outputs,Line()));

        //std::vector<float> genom = net->genom(avtiveAnimal);
        ...
        {
            inputNeuron[a].pos(QPoint(R,(a+1)*(/*D+*/_draw_abstandY)/*+R*/+inputYoffset));
            inputNeuron[a].size(R);
            if(gohstNet_Net <0)
            {
              inputNeuron[a].color(weightColor(net->input(a),1,1));
            }
            else
            {
        ...
        for(int a=0; a<_hiddenX; a++)
        {
            ...
            {
                hiddenNeuron[a][b].pos(QPoint((a+1)*(/*D+*/_draw_abstandX)/*+R*/,(b+1)*(/*D+*/_draw_abstandY)/*+R*/+hiddenYoffset));
                hiddenNeuron[a][b].size(R);

                if(gohstNet_Net <0)
                {
                  hiddenNeuron[a][b].color(weightColor(net->hiddenLayer[a][b].output(),1,1));
                }
                else
                {
        ...
        {
            outputNeuron[a].pos(QPoint(/*D+*/_draw_abstandX+hiddenNeuron[hiddenNeuron.size()-1][0].pos().x(),(a+1)*(/*D+*/_draw_abstandY)/*+R*/+outputYoffset));
            outputNeuron[a].size(R);
            //qDebug("%i x: %i y: %i",a,outputNeuron[a].pos().x(),outputNeuron[a].pos().y());

            if(gohstNet_Net <0)
            {
              outputNeuron[a].color(weightColor(net->outputLayer[a].output(),1,1));
            }
            else
            {
        ...
        for(int a=0; a<inputConnection.size(); a++)//Alle input Seiten
        {
            for(int b=0; b<inputConnection[0].size(); b++)//Alle hiddenY seiten von hiddenX=0
            {
                ...
                else
                {
                    ...
                    if(gohstNet_Net <0)
                    {
                      inputConnection[a][b].color(weightColor(net->input(a),net->hiddenLayer[0][b].weight(a),colorMultiplayer));
                    }
                    else
                    {
            ...
            for(int b=0; b<hiddenConnection[0].size(); b++)//alle x+bias davon
            {
                for(int c=0; c<hiddenConnection[0][0].size(); c++)//alle nächsten
                {
                    if(b == _hiddenY)
                    {
                        ...
                        if(gohstNet_Net <0)
                        {
                           hiddenConnection[a][b][c].color(weightColor(1,net->hiddenLayer[a+1][c].weight(b),colorMultiplayer));
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if(gohstNet_Net <0)
                        {
                           hiddenConnection[a][b][c].color(weightColor(net->hiddenLayer[a][b].output(),net->hiddenLayer[a+1][c].weight(b),colorMultiplayer));
                        }
                        else
                        {
        ...
        for(int a=0; a<outputConnection.size(); a++)
        {
            for(int b=0; b<outputConnection[0].size(); b++)
            {
                if(a == _hiddenY)
                {
                    ...
                    if(gohstNet_Net <0)
                    {
                        outputConnection[a][b].color(weightColor(1,net->outputLayer[b].weight(a),colorMultiplayer));
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if(gohstNet_Net <0)
                    {
                        outputConnection[a][b].color(weightColor(net->hiddenLayer[net->hiddenLayer.size()-1][a].output(),net->outputLayer[b].weight(a),colorMultiplayer));
                    }
                    else
                    {
